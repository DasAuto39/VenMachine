      ...
      <div className="relative z-10">
        ...
        {notification && (
          <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 animate-bounce-short">
            <div className={`backdrop-blur-md px-6 py-3 rounded-none-full font-bold shadow-xl border flex items-center gap-2 ${notification.type === 'error'
        ...
        {gateConfirmPending && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            {/* Card */}
            <div className="relative bg-white rounded-none-none shadow-2xl shadow-slate-900/20 border border-slate-100 max-w-sm w-full p-7 animate-in">
              {/* Icon */}
              <div className="w-14 h-14 bg-amber-50 border-2 border-amber-200 rounded-none-none flex items-center justify-center mx-auto mb-5">
              ...
              <p className="text-center text-sm text-slate-500 font-medium mb-5 leading-relaxed">
                Anda memilih <span className="font-bold text-teal-700">{gateConfirmPending.label}</span>.
                <br />
                Pastikan Anda <span className="font-bold text-slate-700">benar-benar berada</span> di depan mesin <span className="font-bold text-teal-700">{gateConfirmPending.label}</span>.
              </p>
              {/* Warning box */}
              <div className="bg-amber-50 border border-amber-200 rounded-none-none px-4 py-3 mb-6">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 py-2.5 rounded-none-none text-sm font-bold text-slate-500 bg-slate-100 hover:bg-slate-200 transition-all"
                >
                  Batal
                </button>
                <button
                  ...
                  className="flex-1 py-2.5 rounded-none-none text-sm font-bold text-white bg-gradient-to-r from-teal-500 to-teal-600 hover:from-teal-600 hover:to-teal-700 shadow-md shadow-teal-400/30 transition-all"
                >
        ...
        {currentGate === 'unknown' && (
          <div className="fixed top-[62px] left-0 right-0 z-30 flex justify-center pointer-events-none">
            <div className="bg-gradient-to-r from-rose-500 to-rose-600 text-white text-xs font-semibold px-6 py-1.5 rounded-none-none shadow-lg shadow-rose-400/30 tracking-wide">
            ...
            <div className="flex items-center gap-2 sm:gap-4 md:gap-6 shrink-0">
              ...
              <div className="relative">
                <button
                  ...
                  className={`flex items-center gap-2.5 px-3.5 py-2 rounded-none-none text-sm font-semibold transition-all duration-200 ${currentGate === 'unknown'
                    ? 'bg-rose-50 text-rose-700 border border-rose-200 hover:bg-rose-100'
                    : 'bg-teal-50 text-teal-800 border border-teal-200 hover:bg-teal-100'
                    }`}
                >
                  <span className={`w-2 h-2 rounded-none-full ${currentGate === 'unknown' ? 'bg-rose-400 animate-pulse' : 'bg-teal-400'
                    }`} />
                  <span>
                    {currentGate === 'unknown' ? 'Pilih gate...' : currentGate.replace('gate_', 'Gate ')}
                  </span>
                  <svg className={`w-3.5 h-3.5 transition-transform duration-200 ${gateDropdownOpen ? 'rotate-180' : ''} ${currentGate === 'unknown' ? 'text-rose-400' : 'text-teal-500'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 9l-7 7-7-7" /></svg>
                </button>
                ...
                {gateDropdownOpen && (
                  <>
                    ...
                    {/* Panel */}
                    <div className="absolute left-0 top-full mt-2 z-20 bg-white rounded-none-none shadow-xl shadow-slate-200/80 border border-slate-100 overflow-hidden w-52">
                      ...
                      {[{ val: 'gate_1', label: 'Gate 1', desc: 'Keluaran Tengah' }, { val: 'gate_2', label: 'Gate 2', desc: 'Keluaran Kiri' }, { val: 'gate_3', label: 'Gate 3', desc: 'Keluaran Kanan' }].map(g => (
                        ...
                        >
                          <span className={`w-2.5 h-2.5 rounded-none-full shrink-0 ${currentGate === g.val ? 'bg-teal-500' : 'bg-slate-200'}`} />
                          <div>
                            <p className="text-sm font-bold leading-tight">{g.label}</p>
                            <p className="text-xs text-slate-400 font-medium">{g.desc}</p>
                          </div>
                          {currentGate === g.val && (
                            <svg className="ml-auto w-4 h-4 text-teal-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>
                          )}
                        </button>
                      ))}
            ...
            <div className="flex items-center gap-3 overflow-x-auto hide-scrollbar snap-x snap-mandatory sm:overflow-visible max-w-[55vw] sm:max-w-none pl-1 py-1">
              ...
              {user?.role === 'admin' && (
                <button
                  ...
                  className="shrink-0 snap-start px-4 py-2 rounded-none-none text-sm font-bold bg-white text-teal-700 border border-teal-100 hover:bg-teal-50 hover:border-teal-300 shadow-sm transition-all hover:shadow-md hover:-translate-y-0.5 whitespace-nowrap"
                >
                  Dashboard Admin
                </button>
              )}
              <button
                ...
                className="shrink-0 snap-start px-4 py-2 rounded-none-none text-sm font-bold bg-white text-slate-600 border border-slate-200 hover:bg-slate-50 hover:border-slate-300 shadow-sm transition-all hover:shadow-md hover:-translate-y-0.5 whitespace-nowrap"
              >
                Info &amp; Promo
              </button>
              <button
                onClick={() => setIsCartOpen(true)}
                className="shrink-0 snap-start relative px-5 py-2 rounded-none-none text-sm font-bold bg-gradient-to-r from-teal-500 to-teal-500 hover:from-teal-400 hover:to-teal-400 text-white shadow-md shadow-teal-500/30 transition-all hover:shadow-lg hover:-translate-y-0.5 whitespace-nowrap flex items-center gap-2"
              >
                <ShoppingCart size={16} />
                Keranjang
                {cartItemCount > 0 && (
                  <span className="absolute -top-2 -right-2 bg-rose-500 text-white text-[10px] font-black min-w-[20px] min-h-[20px] flex items-center justify-center rounded-none-full border-2 border-white shadow-sm">
                    {cartItemCount}
                  </span>
                )}
              </button>
              {user ? (
                <button
                  ...
                  className="shrink-0 snap-start px-4 py-2 rounded-none-none text-sm font-bold text-rose-500 bg-white border border-rose-100 hover:bg-rose-50 hover:border-rose-200 shadow-sm transition-all hover:shadow-md hover:-translate-y-0.5 whitespace-nowrap"
                >
                  Keluar
                </button>
              ) : (
                <button
                  ...
                  className="shrink-0 snap-start px-5 py-2 rounded-none-none text-sm font-bold bg-gradient-to-r from-teal-500 to-teal-500 text-white shadow-md shadow-teal-500/30 transition-all hover:shadow-lg hover:-translate-y-0.5 whitespace-nowrap"
                >
                  Masuk
                </button>
              )}
        ...
        <main className="max-w-7xl mx-auto px-6 py-8 pt-28">
          ...
          {/* 2. HERO BANNER */}
          <div className="relative overflow-hidden bg-gradient-to-br from-teal-800 via-teal-600 to-teal-700 rounded-none-[2.5rem] p-12 md:p-16 text-white mb-12 shadow-[0_20px_60px_-15px_rgba(16,185,129,0.4)]">
            <div className="absolute top-0 right-0 w-full h-full bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-10"></div>
            <div className="absolute top-0 right-0 -mt-20 -mr-20 w-[30rem] h-[30rem] bg-teal-400/30 rounded-none-full blur-[100px] animate-float"></div>
            <div className="absolute bottom-0 left-0 -mb-20 -ml-20 w-[25rem] h-[25rem] bg-teal-300/20 rounded-none-full blur-[80px] animate-float-reverse"></div>
            <div className="relative z-10 text-center max-w-3xl mx-auto">
              <span className="inline-block py-2 px-5 rounded-none-full bg-white/10 border border-white/20 text-xs font-bold tracking-widest mb-6 uppercase backdrop-blur-md shadow-lg shadow-black/5">
                Smart Minimarket Drive Thru
              </span>
              <h1 className="text-5xl md:text-7xl font-black mb-6 leading-[1.1] tracking-tight text-white drop-shadow-md">Belanja Cepat,<br />Tanpa Ribet.</h1>
              <p className="text-lg md:text-xl text-teal-50 max-w-xl mx-auto font-medium leading-relaxed drop-shadow-sm">Pilih produk favorit Anda dari layar cerdas kami, bayar dengan mudah, dan bawa pulang kesegarannya dalam hitungan detik.</p>
            </div>
          </div>

          {/* Gate location warning card - Overlaps Hero Banner */}
          <div className={`relative z-20 -mt-10 md:-mt-20 mx-auto max-w-4xl mb-8 rounded-none-none border p-5 md:px-6 md:py-5 flex flex-col sm:flex-row items-start sm:items-center gap-4 md:gap-5 transition-all shadow-[0_8px_30px_rgb(0,0,0,0.08)] backdrop-blur-xl ${currentGate === 'unknown'
            ? 'bg-rose-50/90 border-rose-200'
            : 'bg-amber-50/90 border-amber-200/80'
            }`}>
            <div className="flex gap-4 items-start flex-1">
              <div className={`w-12 h-12 rounded-none-none flex items-center justify-center shrink-0 mt-0.5 ${currentGate === 'unknown' ? 'bg-rose-100' : 'bg-amber-100'
                }`}>
            ...
            {currentGate !== 'unknown' && (
              <button
                ...
                className="w-full sm:w-auto shrink-0 px-5 py-2.5 rounded-none-none text-sm font-bold bg-white text-amber-700 border border-amber-200 hover:bg-amber-100 transition-all whitespace-nowrap mt-2 sm:mt-0 shadow-sm"
              >
                Ganti Gate
              </button>
            )}
          ...
          <div className="relative group mb-12 max-w-3xl mx-auto w-full z-10 px-6 md:px-0">
            <div className="absolute inset-0 bg-teal-100/30 blur-xl rounded-none-full"></div>
            <div className="relative bg-white/80 backdrop-blur-xl border border-slate-200 p-2 rounded-none-full shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex items-center transition-all duration-300 focus-within:shadow-[0_8px_40px_rgba(16,185,129,0.15)] focus-within:bg-white focus-within:border-teal-200">
              <div className="pl-6 pr-3 text-teal-500">
              ...
              />
              <button className="bg-gradient-to-r from-teal-500 to-teal-500 hover:from-teal-400 hover:to-teal-400 text-white px-8 py-3 rounded-none-full font-bold shadow-md shadow-teal-500/25 transition-all hover:shadow-lg hover:-translate-y-0.5 shrink-0 hidden sm:block">
                Cari
              </button>
            </div>
          </div>
              ...
              {categories.map((cat, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`shrink-0 px-6 py-3 font-bold rounded-none-none transition-all duration-300 ${isSelected
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
              {filteredItems.length === 0 ? (
                ...
                filteredItems.map((item) => (
                  <div
                    key={item.id}
                    className={`bg-white/70 backdrop-blur-xl border border-white rounded-none-[2rem] p-4 flex flex-col transition-all duration-500 group ${item.machine_stock === 0
                  ...
                  >
                    ...
                    <div className="relative w-full aspect-[4/3] bg-gradient-to-br from-slate-100 to-slate-50 rounded-none-none mb-5 flex items-center justify-center group-hover:shadow-inner transition-all overflow-hidden text-slate-300">
                      {item.image_url ? (
                        <img
                      ...
                      {item.description && item.machine_stock > 0 && (
                        <div className="absolute inset-0 bg-teal-900/80 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center p-4 text-center z-10 pointer-events-none translate-y-2 group-hover:translate-y-0">
                      ...
                      {item.machine_stock === 0 && (
                        <div className="absolute inset-0 bg-slate-900/40 rounded-none-none flex flex-col items-center justify-center gap-1 backdrop-blur-[2px] z-20">
                          <span className="text-white font-black text-xs bg-rose-600 px-4 py-1.5 rounded-none-full tracking-wider shadow-lg">STOK HABIS</span>
                        </div>
                      )}
                    </div>

                    <div className="px-2 flex-1 flex flex-col">
                      <h3 className="text-base font-bold text-slate-800 line-clamp-2 leading-snug mb-2 group-hover:text-teal-600 transition-colors">{item.name}</h3>
                      ...
                      <div className="mt-auto pt-2 mb-4 border-b border-slate-100 pb-3">
                        <div className="flex flex-col">
                          ...
                          <div className="flex flex-wrap items-center justify-between gap-1.5">
                            <p className="text-lg font-black text-teal-500 drop-shadow-sm leading-none break-words">
                              Rp {(item.price || 0).toLocaleString('id-ID')}
                            </p>
                            <span className={`px-2 py-0.5 rounded-none text-[10px] font-black uppercase tracking-wider ${item.machine_stock > 5 ? 'bg-slate-100 text-slate-600' : 'bg-rose-100 text-rose-600'}`}>
                      ...
                      <button
                        ...
                        className={`w-full rounded-none-none flex items-center justify-center font-bold py-3 transition-all gap-2 ${item.machine_stock === 0
                          ? 'bg-slate-100 text-slate-400 border border-slate-200 cursor-not-allowed'
                          : 'bg-gradient-to-r from-teal-500 to-teal-500 text-white hover:from-teal-400 hover:to-teal-400 active:scale-95 shadow-lg shadow-teal-500/20'
                          }`}
                      >
                        {item.machine_stock === 0 ? (
          ...
          <div className="fixed inset-0 z-50 flex justify-end">
            ...
            <div className="relative w-full md:w-[420px] bg-white h-full shadow-2xl flex flex-col animate-slide-in border-l border-slate-200">
              <div className="p-6 border-b border-slate-100 flex justify-between items-center">
                ...
                <button
                  onClick={() => setIsCartOpen(false)}
                  className="w-10 h-10 bg-slate-100 hover:bg-rose-100 text-slate-500 hover:text-rose-500 rounded-none-full flex items-center justify-center transition-colors"
                >
                  ✕
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-3">
                {cartItemCount === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12 text-slate-400">
                    <div className="w-24 h-24 bg-slate-50 rounded-none-full flex items-center justify-center text-slate-300 mb-4">
                  ...
                  Object.entries(cart).map(([itemId, { item, qty }]) => (
                    <div key={itemId} className="bg-white border border-slate-200 p-3 rounded-none-none shadow-sm">
                      <div className="flex justify-between items-start mb-2">
                        <div className="flex items-center gap-3 flex-1">
                          <div className="w-12 h-12 bg-slate-50 rounded-none-none flex items-center justify-center text-xl shrink-0 text-slate-400 overflow-hidden relative">
                            {item.image_url ? (
                              <img
                                src={item.image_url.includes('/uploads/') ? `${import.meta.env.VITE_API_BASE_URL}${item.image_url.substring(item.image_url.indexOf('/uploads/'))}` : item.image_url}
                                alt={item.name}
                          ...
                          <div>
                            ...
                            <p className="text-xs font-semibold text-teal-600 mt-0.5">Rp {(item.price || 0).toLocaleString('id-ID')}</p>
                          </div>
                        </div>
                        <button
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-slate-100">
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="w-7 h-7 bg-slate-200 hover:bg-rose-200 text-slate-700 hover:text-rose-700 rounded-none-none flex items-center justify-center transition-colors text-sm font-bold"
                          >
                            −
                          </button>
                          ...
                          <button
                            ...
                            className="w-7 h-7 bg-slate-200 hover:bg-teal-200 text-slate-700 hover:text-teal-700 rounded-none-none flex items-center justify-center transition-colors text-sm font-bold"
                          >
                            +
                          </button>
                        </div>
              ...
              <div className="p-6 bg-white border-t border-slate-200 mt-auto">
                {checkoutError && (
                  <div className="mb-4 p-3 bg-rose-50 border border-rose-200 text-rose-600 text-sm font-semibold rounded-none-none flex items-start gap-2">
                ...
                <div className="flex justify-between items-center pt-4 border-t border-slate-100 mb-6">
                  ...
                  <span className="text-2xl font-black bg-clip-text text-transparent bg-gradient-to-r from-teal-500 to-teal-500">Rp {cartTotal.toLocaleString('id-ID')}</span>
                </div>
                <button
                  onClick={handleCheckout}
                  disabled={cartItemCount === 0}
                  className="w-full bg-gradient-to-r from-teal-500 to-teal-500 hover:from-teal-600 hover:to-teal-600 text-white font-bold py-4 rounded-none-none disabled:from-slate-300 disabled:to-slate-300 disabled:text-slate-500 transition-all duration-300 shadow-lg shadow-teal-500/30 disabled:shadow-none text-lg transform hover:-translate-y-0.5 active:scale-95 flex items-center justify-center gap-2"
                >
        ...
        {isPaymentModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            {/* Modal Panel */}
            <div className="relative bg-white/95 backdrop-blur-xl border border-white/20 rounded-none-[2rem] shadow-2xl p-8 max-w-sm w-full mx-4 transform transition-all">
              ...
              {paymentStatus === 'success' && (
                <div className="text-center py-6">
                  ...
                  <h3 className="text-2xl font-black bg-clip-text text-transparent bg-gradient-to-r from-teal-500 to-teal-500 mb-2">Pembayaran Berhasil!</h3>
              ...
              {paymentStatus === 'failed' && (
                <div className="text-center py-6">
                  ...
                  <button
                    ...
                    className="w-full px-6 py-3 bg-slate-100 text-slate-700 rounded-none-full font-bold hover:bg-slate-200 transition-colors"
                  >
                    Coba Lagi
                  </button>
                </div>
              ...
              {!paymentStatus && isProcessing && (
                <div className="text-center py-6">
                  <div className="flex justify-center mb-6">
                    <Loader2 size={60} className="animate-spin text-teal-500 drop-shadow-md" />
                  </div>
